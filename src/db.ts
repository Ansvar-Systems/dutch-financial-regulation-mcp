/**
 * SQLite database access layer for de Nederlandse Financiële Regelgeving MCP server.
 *
 * Schema volgt de structuur van AFM en DNB regelgeving:
 *   - sourcebooks        — brondocumenten: AFM Leidraden, AFM Beleidsregels, DNB Toezichtregelingen, DNB Good Practices
 *   - provisions         — individuele bepalingen, richtlijnen en beleidsregels
 *   - enforcement_actions — AFM/DNB handhavingsbesluiten en boetes
 *
 * FTS5 virtuele tabellen ondersteunen volledige-tekstzoekfunctie op bepalingen en handhavingsacties.
 */
...
CREATE TABLE IF NOT EXISTS provisions (
  ...
  text           TEXT    NOT NULL,
  type           TEXT,
  status         TEXT    DEFAULT 'van_kracht',
  effective_date TEXT,
  chapter        TEXT,
  section        TEXT,
...
export function getProvision(
  ...
  // Fuzzy fallback: zoek op referentie zonder hoofdletteronderscheid
  return (
    (db
      .prepare(
...
export function checkProvisionCurrency(reference: string): {
  ...
  if (!row) {
    return { reference, status: "onbekend", effective_date: null, found: false };
  }
