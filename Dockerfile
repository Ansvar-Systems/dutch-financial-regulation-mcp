# ─────────────────────────────────────────────────────────────────────────────
# Dutch Financial Regulation MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t dutch-financial-regulation-mcp .
# Run:    docker run --rm -p 3000:3000 dutch-financial-regulation-mcp
#
# The image bakes /app/data/afm.db at build time. Override AFM_DB_PATH
...
ENV AFM_DB_PATH=/app/data/afm.db
...
# Bake the pre-built database into the image so /app/data/afm.db resolves
# at runtime without a bind mount.
#
# The explicit `data/afm.db` reference is required — `.github/workflows/
...
# We then COPY it into the image at /app/data/afm.db (AFM_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/afm.db
